...
<script>
  ...
  var UsersList = {
  	    ...
  	    loadList: function() {
  	        return m.request({
  	            method: "GET",
  	            url: "./_ah/api/myApi/v1/usersList/"})
  	        .then(function(result) {
  	        	console.log("got:",result)
  	        	UsersList.list=result.items
  	            if ('nextPageToken' in result) {
  		        	UsersList.nextToken= result.nextPageToken
  	            } else {
  	            	UsersList.nextToken=""
  	            }})
  	    },
  	    next: function() {
  	        return m.request({
  	            method: "GET",
  	            url: "./_ah/api/myApi/v1/usersList/"+"?next="+UsersList.nextToken})
  	        .then(function(result) {
  	        	console.log("got:",result)
  	        	result.items.map(function(item){UsersList.list.push(item)})
  	            if ('nextPageToken' in result) {
  		        	UsersList.nextToken= result.nextPageToken
  	            } else {
  	            	UsersList.nextToken=""
  ...
  var UsersView = {
    oninit: UsersList.loadList,
    view: function() {
     	return m('div', [
  	  m('div',{class:'subtitle'},"Liste des Utilisateurs"),
  	  m('table', {class:'table is-striped',"table":"is-striped"},[
  	    m('tr', [
  	      ...
  	      m('th', {width:"50px"}, "age"),
  	      m('th', {width:"50px"}, "NbFollowers"),
  	   
  	    ]),
  	    UsersList.list.map(function(item) {
  	      return m("tr", [
  	        ...
  	        //m('td', m('label', item.properties.NbFollowers)),
  	        m("td", m("button", {
	    		  class : 'button is-activered',
	    		  onclick: function(e) {
  	  ...
  	  m("center",m("h1", m("td", m('button',{
	      ...
	      onclick: function(e) {UsersList.next()},"style":{"width":"150vh","height":"5vh"}
	      },
	  "Next")))),
...
//             	        "Cliquer sur le boutton 'Show Users' pour voir la liste des utilisateurs :)"
//             	                ),
            	                
          
      // m("div", {class: 'tile'}, m('div',{class:'tile is-child box'},m(PostForm))),
        	   m("div", {class: 'tile'}, m('div',{class:'tile is-child box'},m(UsersView))),
