...
//   		}
  ...
  var AbonneList = {
  	    ...
  	    loadList: function() {
  	        return m.request({
  	            method: "GET",
  	            url: "./_ah/api/myApi/v1/abonneList/"})
  	        .then(function(result) {
  	        	console.log("got:",result)
  	        	AbonneList.list=result.items
  	            if ('nextPageToken' in result) {
  		        	AbonneList.nextToken= result.nextPageToken
  	            } else {
  	            	AbonneList.nextToken=""
  	            }})
  	    },
  	    next: function() {
  	        return m.request({
  	            method: "GET",
  	            url: "./_ah/api/myApi/v1/abonneList/"+"?next="+AbonneList.nextToken})
  	        .then(function(result) {
  	        	console.log("got:",result)
  	        	result.items.map(function(item){AbonneList.list.push(item)})
  	            if ('nextPageToken' in result) {
  		        	AbonneList.nextToken= result.nextPageToken
  	            } else {
  	            	AbonneList.nextToken=""
     	...
     	showfollowers : function(e){  
    		 window.location = "/paper-dashboard-master/followersPage.jsp";
    	 },
         	 
  }

  var AbonnéView = {
    oninit: AbonneList.loadList,
    view: function() {
     	return m('div', [
  	  m('div',{class:'subtitle'},"Mes abonnements"),
  	  m('table', {class:'table is-striped',"table":"is-striped"},[
  	    ...
  	    AbonneList.list.map(function(item) {
  	      return m("tr", [
  	       ...
  	       // m('td', m('label', item.properties.NbFollowBy)),
  	        ...
  	        m("td", m("button", {class : 'button is-activered',
  	        	...
  	        	onclick: function(e) {
  	        		...
  	        		AbonneList.showfollowers(item.key.id);
...
view: function() {
 ...
 return [m("div", {"class":"wrapper "},
    [
      m("div", {"class":"sidebar","data-color":"grey","data-active-color":"danger"},
        [
          m("div", {"class":"logo"},
            [
              m("a", {"class":"simple-text logo-mini","href":"https://www.creative-tim.com"},
                [
                  m("div", {"class":"logo-image-small"}, 
                    m("img", {"src":"assets/img/logo-small.png"})
                  ),
                  m("p", 
                    "CT"
                  )
          ...
          m("div", {"class":"sidebar-wrapper"}, 
            m("ul", {"class":"nav"},
              [
                ...
                m("li", 
                        m("a", {"href":"/paper-dashboard-master/abonnéPage.jsp"},
                          [
                            m("i", {"class":"nc-icon nc-diamond"}),
                            m("p", 
                              "Mes Abonnés " 
                  ...
                  [
                    ...
                    m("ul", {"class":"navbar-nav"}, 
                      m("li", {"class":"nav-item btn-rotate dropdown"},
                        [
                          m("a", {"class":"nav-link dropdown-toggle","href":"","id":"navbarDropdownMenuLink","data-toggle":"dropdown","aria-haspopup":"true","aria-expanded":"false"},
                            [
                              m("i", {"class":"nc-icon nc-bell-55"}),
                              m("p", 
          ...
          m("div", {"class":"content"}, 
            m("div", {"class":"row"}, 
              m("div", {"class":"col-md-12"}, 
            	m("h3", {"class":"description"}, 
            	        "Cliquer sur le boutton 'Show My Friends' pour voir la liste des utilisateurs :)"
            	                ),
      ...
      // m("div", {class: 'tile'}, m('div',{class:'tile is-child box'},m(PostForm))),
        	   m("div", {class: 'tile'}, m('div',{class:'tile is-child box'},m(AbonnéView))),
